...
if( isset( $_POST['login'] ) ) {
	...
	if( mysqli_num_rows($result) > 0 ) {
		...
		if( $formPass==$hashedPass ) {
			...
			//error message
			$loginError="<div class='alert-danger'>Wrong username / password combination. Try again.</div>";
			
		}
	}
	else { //there are no results in database
		
		$loginError="<div class='alert-danger'>No such user in database. Please Try again.<a class='close' data-dismiss='alert'>&times;</a></div>";
	}
